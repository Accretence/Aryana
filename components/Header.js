...
export default function ({ config, i18n, useThemeProvider, useAuth, useWindowSize }) {
  ...
  const { locale = config.defaultLocale, locales, pathname, asPath, query } = router
  ...
  const LoginModal = () => {
    ...
    const [confirmPassword, setConfirmPassword, refConfirmPassword] = useState('')
    const [loading, setLoading, refLoading] = useState(false)

    return (
      <>
        {config && i18n && buttons && (
          <Modal py={0.2} visible={modalVis} onClose={() => setModalVis(false)}>
            <Modal.Content pt={0.5} pb={2.5}>
              <Collapse.Group>
                <Collapse
                  ...
                  subtitle={
                    <Text small>{header['modal']['login']['subtitle'][locale]}</Text>
                  }>
                  <Input
                    label={!isLocaleRTL(locale) && <Mail size={16} />}
                    labelRight={isLocaleRTL(locale) && <Mail size={16} />}
                  ...
                  />
                  <Input.Password
                    label={!isLocaleRTL(locale) && <Lock size={16} />}
                    labelRight={isLocaleRTL(locale) && <Lock size={16} />}
                ...
                <Collapse
                  ...
                  subtitle={
                    <Text small>{header['modal']['register']['subtitle'][locale]}</Text>
                  }>
                  <Input
                    label={!isLocaleRTL(locale) && <Mail size={16} />}
                    labelRight={isLocaleRTL(locale) && <Mail size={16} />}
                  ...
                  />
                  {!refPassword.current == '' && refPassword.current.length < 8 && (
                  ...
                  <Input.Password
                    ...
                    type={
                      refConfirmPassword.current == ''
                        ? 'default'
                        : refConfirmPassword.current.length > 7 &&
                          refConfirmPassword.current == refPassword.current
                        ? 'success'
                        : 'error'
                    }
                    width="100%"
                  ...
                  {!refConfirmPassword.current == '' &&
                    refConfirmPassword.current.length < 8 && (
                      <Text small type="error">
                        {i18n['inputs']['confirmPassword']['error'][locale]}{' '}
                      </Text>
                    )}
                  {!refConfirmPassword.current == '' &&
                    refConfirmPassword.current != refPassword.current && (
        ...
        <>
          <Text mt={1.5} className="MenuNavigationTitle">
            <Link className="MenuNavigationTitle" href="/">
              {i18n['components']['header']['title'][locale].toUpperCase()}
            </Link>
          </Text>
        </>
      )}
      ...
      <div>
        ...
        {themeProvider && (
          <Button
            ...
            auto
            onClick={() =>
              themeProvider.setLocalTheme(theme.type === 'dark' ? 'light' : 'dark')
            }
          />
        )}
        {/* {locales && (
  ...
  const PhoneNav = () => (
    <>
      <Button type="secondary" ghost style={{ border: 'none' }} auto icon={<Search />} />
      <Title config={config} i18n={i18n} />
      <Button
        type="secondary"
        ghost
      ...
      <Drawer
        ...
        <Drawer.Content>
          {
            <ButtonGroup type="secondary" mx={0} mb={2} width="100%">
              <Button
        ...
        <>
          ...
          <style jsx global>
            {`
              ...
              .MainDropdown {
                margin-left: ${isLocaleRTL(locale) ? '' : '0.5rem'};
                margin-right: ${isLocaleRTL(locale) ? '0.5rem' : ''};
              }
              .MainDropdown > button {
                white-space: nowrap;
              }
